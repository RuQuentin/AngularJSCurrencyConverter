          ...
          <tbody>
            <tr class="table-row" ng-repeat="el in data | orderBy:sort.sortingOrder:sort.reverse">
              ...
              <td ng-show='adminTable'>
                <input type="checkbox" ng-checked="value.role === 'admin'" ng-click='changeUserRole({data:el.id})'>
              </td>
              <td ng-show='adminTable'>
                <button class="btn btn-outline-secondary" type="button" ng-click='resetPsw({data:el.id})'>Reset
                  password</button>
              </td>

              <td ng-show='adminTable'><i class="fas fa-history tabble__icon" data-toggle="modal"
                  ng-click="setSelectedUser({data:el.id})" data-target="#userHistoryModal"></i></td>
